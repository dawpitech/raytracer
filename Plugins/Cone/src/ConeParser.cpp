...
std::unique_ptr<raytracer::engine::IObject> raytracer::engine::objects::cone::ConeParser::parseObject(const libconfig::Setting& objectConfig)
{
  try {
    ...
    if (objectConfig.exists("height")) {
      height = static_cast<double>(objectConfig.lookup("height"));
    }

    return std::make_unique<Cone>(math::Point3D{px, py, pz}, math::Vec3<double>{dx, dy, dz}, angle, height);
